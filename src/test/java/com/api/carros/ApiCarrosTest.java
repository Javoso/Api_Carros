...
public class ApiCarrosTest {
	...
	private ResponseEntity<CarroDTO> getCarro(String url) {
		return restTemplate.withBasicAuth("lucas", "123").getForEntity(url, CarroDTO.class);
	}

	private ResponseEntity<List<CarroDTO>> getCarros(String url) {
		return restTemplate.withBasicAuth("lucas", "123").exchange(url, HttpMethod.GET, null, new ParameterizedTypeReference<List<CarroDTO>>() {
		});
	}
	...
	public void testEndPointSalvar() {
		...
		ResponseEntity response = restTemplate.withBasicAuth("javoso","123").postForEntity("/api/v1/carros/", carro, null);
		...
		restTemplate.withBasicAuth("javoso", "123").delete(location);
